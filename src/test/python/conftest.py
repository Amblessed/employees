...
# @pytest.fixture(scope="session", autouse=True)
# def wait_for_fresh_user_details():
#     print("⏳ Waiting for fresh user_details.json...")
#     last_seen = USER_DETAILS_PATH.stat().st_mtime if USER_DETAILS_PATH.exists() else 0
#
#     for _ in range(150):  # wait up to 5 minutes
#         if USER_DETAILS_PATH.exists():
#             current = USER_DETAILS_PATH.stat().st_mtime
#             if current > last_seen:
#                 print("✅ Fresh user_details.json detected!")
#                 return
#         time.sleep(2)
#
#     pytest.exit("❌ Timeout: Fresh user_details.json was not generated.")