...
def wait_for_seeder_json(timeout=60):
    ...
    print("Waiting for user_details.json to be generated by seeder...")
    for _ in range(timeout):
        if USER_DETAILS_FILE.exists() and USER_DETAILS_FILE.stat().st_size > 0:
            print("Seeder JSON found!")
            return
        time.sleep(1)
    raise FileNotFoundError(f"{USER_DETAILS_FILE} not found after {timeout} seconds")
